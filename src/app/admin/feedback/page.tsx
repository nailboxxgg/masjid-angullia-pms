    ...
    return (
        ...
        >
            <motion.div variants={itemVariants} className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-secondary-900 dark:text-white uppercase tracking-tighter">Community Inbox</h1>
                    <p className="text-secondary-900 dark:text-secondary-200 mt-1 font-medium italic">Centrally manage community messages, inquiries, and service requests.</p>
                </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
                <Card className="bg-white dark:bg-secondary-900 border-secondary-200 dark:border-secondary-800 shadow-sm overflow-hidden rounded-2xl">
                    <CardHeader className="pb-3 border-b border-secondary-100 dark:border-secondary-800 bg-secondary-50/30 dark:bg-secondary-800/20">
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-20">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={cn(
                                        "flex items-center gap-2 px-6 h-11 border-none ring-1 rounded-xl text-sm font-bold transition-all shadow-sm",
                                ...
                                <AnimatePresence>
                                    {isFilterOpen && (
                                        <motion.div
                                            ...
                                            className="absolute right-0 top-full mt-3 w-72 bg-white dark:bg-secondary-900 rounded-2xl shadow-2xl border border-secondary-200 dark:border-secondary-800 p-5 space-y-4 z-50"
                                        >
                    ...
                    <CardContent className="pt-6 px-0">
                        <div className="px-6 pb-2">
                            {isLoading ? (
                                ...
                                <div className="bg-secondary-50 dark:bg-secondary-800/20 rounded-3xl border-2 border-dashed border-secondary-200 dark:border-secondary-800 p-20 text-center">
                                    <div className="w-20 h-20 bg-secondary-100 dark:bg-secondary-800 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner ring-1 ring-secondary-200 dark:ring-secondary-700">
                                        <MessageSquare className="w-10 h-10 text-secondary-300 dark:text-secondary-600" />
                                    </div>
                                    <h3 className="text-2xl font-black text-secondary-900 dark:text-white uppercase tracking-tight">Your Inbox is Clear</h3>
                                    <p className="text-secondary-500 font-medium italic mt-2">No community submissions match your current filters.</p>
                                </div>
                            ) : (
                                    ...
                                    <AnimatePresence mode="popLayout">
                                        {filteredFeedbacks.map((item, index) => (
                                            <motion.div
                                                ...
                                                className="p-6 rounded-3xl border border-secondary-100 dark:border-secondary-800 hover:shadow-2xl transition-all group cursor-pointer bg-white dark:bg-secondary-900 hover:border-primary-200 dark:hover:border-primary-900/50 relative overflow-hidden"
                                            >
                                                {item.status === 'New' && (
                                                ...
                                                <div className="flex flex-col md:flex-row gap-6 items-start justify-between">
                                                    <div className="flex items-start gap-5">
                                                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 font-black bg-secondary-50 text-secondary-900 dark:bg-secondary-800 dark:text-white text-xl shadow-inner ring-1 ring-secondary-100 dark:ring-secondary-700/50 group-hover:bg-primary-50 group-hover:text-primary-600 dark:group-hover:bg-primary-900/20 dark:group-hover:text-primary-400 transition-colors">
                                                            {item.name?.charAt(0) || '?'}
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-3 mb-1.5 focus:outline-none">
                                                                <h4 className="font-bold text-secondary-900 dark:text-secondary-100 text-xl group-hover:text-black dark:group-hover:text-white transition-colors tracking-tight">{item.name || 'Anonymous Submission'}</h4>
                                                                {item.status === 'New' && (
                                                                    <span className="flex h-2.5 w-2.5 rounded-full bg-primary-500 ring-4 ring-primary-500/20 animate-pulse" />
                                                                )}
                                                            </div>
                                                            <div className="flex flex-wrap items-center gap-4 text-[13px] font-bold text-secondary-500">
                                                                {item.email && (
                                                                    <span className="flex items-center gap-2 group-hover:text-secondary-700 dark:group-hover:text-secondary-300 transition-colors">
                                                                        <Mail className="w-4 h-4 text-primary-500/60" /> {item.email}
                                                                    </span>
                                                                )}
                                                                {item.contactNumber && (
                                                                    <span className="flex items-center gap-2 group-hover:text-secondary-700 dark:group-hover:text-secondary-300 transition-colors">
                                                                        <Phone className="w-4 h-4 text-emerald-500/60" /> {item.contactNumber}
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        ...
                                                        <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-secondary-400 bg-secondary-50 dark:bg-secondary-800/50 px-3 py-1.5 rounded-xl border border-secondary-100 dark:border-secondary-800">
                                                            <Clock className="w-3.5 h-3.5" />
                                                            {item.createdAt?.toDate ? new Date(item.createdAt.toDate()).toLocaleDateString('en-PH', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Pending'}
                                                        </span>
                                                    </div>
                                                </div>

                                                <div className="mt-6 ml-0 md:ml-16">
                                                    <div className="p-5 rounded-2xl bg-secondary-50/50 dark:bg-secondary-950 text-secondary-800 dark:text-secondary-200 font-medium text-sm leading-relaxed line-clamp-2 border border-secondary-100 dark:border-secondary-800 shadow-inner group-hover:bg-white dark:group-hover:bg-secondary-900 transition-all">
                                                        {item.message}
                                                    </div>
                    ...
                    <div className="space-y-8">
                        <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 border-b border-secondary-100 dark:border-secondary-800 pb-8">
                            <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
                                <div className="w-20 h-20 rounded-3xl bg-primary-50 dark:bg-primary-900/30 flex items-center justify-center shrink-0 text-primary-600 dark:text-primary-400 font-black text-3xl shadow-xl ring-4 ring-primary-500/10">
                                    {selectedFeedback.name?.charAt(0) || '?'}
                                </div>
                                <div>
                                    <h4 className="font-black text-secondary-900 dark:text-white text-3xl tracking-tight">{selectedFeedback.name || 'Anonymous'}</h4>
                            ...
                            <div className="flex flex-col items-center sm:items-end gap-3 text-center sm:text-right">
                                <span className={cn(
                                    "px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-md",
                                ...
                                <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-secondary-400 italic">
                                    <Clock className="w-4 h-4" /> {selectedFeedback.createdAt?.toDate ? new Date(selectedFeedback.createdAt.toDate()).toLocaleString('en-PH', { dateStyle: 'full', timeStyle: 'short' }) : "Pending"}
                                </span>
                            </div>
                        </div>

                        <div className="bg-secondary-50/50 dark:bg-secondary-950 p-8 rounded-3xl border border-secondary-100 dark:border-secondary-800 shadow-inner">
                            ...
                            <p className="text-secondary-800 dark:text-secondary-100 whitespace-pre-wrap leading-relaxed font-bold text-lg italic">
                        ...
                        <div className="flex justify-end gap-4 pt-4">
                            <button
                                ...
                                className="px-8 py-3.5 border-none ring-1 ring-secondary-200 dark:ring-secondary-700 rounded-2xl text-secondary-900 dark:text-secondary-100 hover:bg-secondary-50 dark:hover:bg-secondary-800 text-xs font-black uppercase tracking-widest transition-all"
                            >
                                Dismiss
                            </button>
                            {selectedFeedback.status !== 'Resolved' && (
                                <motion.button
                                    ...
                                    className="px-8 py-3.5 bg-emerald-600 dark:bg-emerald-700 rounded-2xl text-white hover:bg-emerald-700 dark:hover:bg-emerald-800 text-xs font-black uppercase tracking-widest shadow-xl shadow-emerald-600/20 transition-all flex items-center gap-2"
                                >
